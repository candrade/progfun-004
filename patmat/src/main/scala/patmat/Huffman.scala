...
object Huffman {
  ...
  /**
   * Helper function for inserting a new item in a sorted fashion dependent on
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case Nil => List()
    case y :: ys => leafListInsert(Leaf(y._1, y._2), makeOrderedLeafList(ys))
  } 
   ...
   */
  def until(isSingleton: List[CodeTree] => Boolean, combiner: List[CodeTree] => List[CodeTree])(treeList: List[CodeTree]): List[CodeTree] = 
    if (isSingleton(treeList)) treeList
    else until(isSingleton, combiner)(combiner(treeList))
